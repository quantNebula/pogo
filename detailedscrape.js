...
function main()
{
    ...
    try
    {
        events.forEach(e => {
            ...
            generic.get(e.link, e.eventID, events);
            // get event type specific extra data
            if (e.eventType == "research-breakthrough")
            {
                breakthrough.get(e.link, e.eventID, events);
            }
            else if (e.eventType == "pokemon-spotlight-hour")
            {
                spotlight.get(e.link, e.eventID, events);
            }
            else if (e.eventType == "community-day")
            {
                communityday.get(e.link, e.eventID, events);
            }
            else if (e.eventType == "raid-battles")
            {
                raidbattles.get(e.link, e.eventID, events);
            }
            else if (e.eventType == "research")
            {
                research.get(e.link, e.eventID, events);
            }
        });
    }
    catch (error)
